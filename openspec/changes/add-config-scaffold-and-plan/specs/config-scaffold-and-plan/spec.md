...
Reduce first-run friction by scaffolding `mask-rules.yaml` from a live source schema and
previewing the masking plan without requiring a target database or writes.
...
None — `dry-run` behavior unchanged; new commands only.
